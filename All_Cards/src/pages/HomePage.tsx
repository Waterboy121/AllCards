...
function HomePage() {
  ...
  const [collectionCopy, setCollectionCopy] = useState<UserCollection[]>([]); //used for checking, if collections need to be updated in the database
  ...
  const handleChangeUserCollection = (collectionTabs: UserCollection[]) => {
    setCollectionTabs(collectionTabs);
  };

  const handleChangeCollectionCopy = (collectionTabs: UserCollection[]) => {
    setCollectionCopy(collectionTabs);
  };

  const handleCreateCollection = (name: string, franchiseKey: string) => {
    if (collectionTabs.find((col) => col.name === name)) return; //so collections don't have the same name
  ...
  return (
    <>
      <div className="homepage-container">
        <NavBar changeSearchValue={changeSearchValue} />
        <div className="homepage-layout">
          <Sidebar
            collections={collectionTabs}
            collectionsCopy={collectionCopy}
            currentTab={currentTab}
            onTabClick={setCurrentTab}
            onAddCollection={handleAddCollection}
            onAddCard={handleAddCard}
            handleChangeUserCollection={handleChangeUserCollection}
            handleChangeCollectionCopy={handleChangeCollectionCopy}
